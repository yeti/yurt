...
module "auth0_tenant" {
  ...
  # The production frontend's URL. Replace after the Render static site exists
  # (Phase 3 step 4, infra/README.md) — usually
  # https://<project>-production-frontend.onrender.com (Render only appends a
  # random suffix on a name collision).
  app_url = "https://REPLACE_ME.onrender.com"

  # No additional_app_urls: local dev runs against the staging tenant only.
